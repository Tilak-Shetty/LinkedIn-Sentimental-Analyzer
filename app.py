...
""")
    ...
    print(req1.json)
    if st.button("Extract Information"):
        article_text = article_data.get('output', [{'contents': [{'utterance': ''}]}])[0]['contents'][0]['utterance']
...
if choose == "Competitor Analysis":
    ...
    if st.button("Extract Information"):
        ...
        res1 =  requests.post(url, headers={"Authorization": f"Bearer {token}"}, json=body)
        data = res1.json()
        if('output') in article_data:
             article_text = article_data['output'][0]['contents'][0]['utterance']
        else:
        # Handle the case when 'output' key is not present
            print("'output' key not found in article_data")
        message_placeholder = st.empty()
        full_response = ""
        with st.chat_message("assistant"):
                st.markdown(assistant_response)
